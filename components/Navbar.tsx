...
import { Search, User, Heart, ShoppingBag, ChevronDown, ChevronUp } from 'lucide-react';
        ...
        <div className="flex items-center space-x-3 sm:space-x-4">
          <form onSubmit={handleSearch} className="relative hidden sm:block">
            ...
            <input
              ...
              className="pl-9 pr-4 py-1 text-white bg-transparent border border-white/50 rounded-lg focus:ring-2 focus:ring-white focus:border-transparent w-32 sm:w-40"
              value={searchQuery}
        ...
        <div className="md:hidden absolute top-full left-0 right-0 bg-black/95 backdrop-blur-md">
          <div className="px-6 py-4 space-y-4" role="menu">
            {NAV_ITEMS.map((item: NavItem, index: number) => (
              <div key={index}>
                <button
                  className="flex justify-between items-center w-full text-white/90 hover:text-white transition-colors duration-200 py-2"
                  role="menuitem"
                  onClick={() => item.hasDropdown ? toggleMobileItem(index) : handleNavItemClick(item.href)}
                >
                  <span>{item.label}</span>
                  {item.hasDropdown && (
